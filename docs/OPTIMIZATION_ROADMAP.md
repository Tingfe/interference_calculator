...
- 第一阶段的核心性能目标已经通过 NumPy 向量化重写落地，`interference()` 不再依赖逐条 `Molecule()` / pyparsing 解析。
- 旧的 `use_pruning`、`n_workers`、`use_streaming` 参数保留为兼容入口，但不再是 2.7 主路径的性能关键。
- 2.8 阶段已引入样品画像先验，用于把基体、主要成分、痕量杂质、背景气体和等离子体来源纳入干扰风险排序。
- 下一阶段重点应转向：形成因子实测校准、样品画像验证数据集、以及和独立 GDMS 数据分析软件的数据接口稳定化。
