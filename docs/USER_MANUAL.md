# Interference Calculator 2.4.0 图文用户手册
...
- 导入元素（N）：仅在导入 GDMS Excel 文件后出现，用于补回本次文件中提取但当前缺失的元素。
- Ar 等离子体背景
- 轻元素背景
- 卤素 / 硫
- 过渡金属基体
...
- 目标峰位于中心。
- ppm 模式下横轴为 `Δppm`。
- 如果已导入 GDMS Excel，可在谱图工具栏手动打开 `实测峰（测试）`，叠加文件中的真实 `Mass` / `Values` 峰形。该功能默认关闭。
